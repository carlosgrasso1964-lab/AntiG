...
// Carrega fornecedores (codCliFor < 1000)
$stmtFor = $db->query("SELECT codCliFor, apelidoCliFor, fkCliForGp FROM tbclifor WHERE codCliFor < '1000' ORDER BY apelidoCliFor");
$fornecedores = $stmtFor->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($fornecedorSelecionado && $dataIni && $dataFim) {
        ...
        try {
            ...
            $stmtF->execute([$fornecedorSelecionado]);
            $fornecedorInfo = $stmtF->fetch();
            $fornecedorNome = $fornecedorInfo ? $fornecedorInfo['apelidoCliFor'] : '';
            
            // Saldo anterior (espelha PegaSaldoAnterior do VConsFornecedores.java — sem filtro 9.*)
            $stmtSaldo = $db->prepare("SELECT COALESCE(SUM(Valor), 0) FROM tbmovimento WHERE dtVcto < ? AND clifor = ? AND recurso = '0079'");
            $stmtSaldo->execute([$dataIniSQL, $fornecedorSelecionado]);
            $saldoAnterior = (float)$stmtSaldo->fetchColumn();

            // Ficha financeira completa: inclui tanto entradas (+) quanto baixas (classif 9.*, -)
            $sql = "SELECT * FROM tbmovimento WHERE dtVcto BETWEEN ? AND ? AND clifor = ? AND recurso = '0079' ORDER BY dtVcto, idMov";
            $stmtMov = $db->prepare($sql);
            $stmtMov->execute([$dataIniSQL, $dataFimSQL, $fornecedorSelecionado]);
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4 class="page-title mb-0"><i class="bi bi-people"></i> Fornecedores</h4>
</div>
    ...
    <div class="col-md-3">
        ...
        <select name="fornecedor" class="form-select" required>
            ...
            <?php foreach ($fornecedores as $f): ?>
                <option value="<?= htmlspecialchars($f['codCliFor']) ?>" <?= $fornecedorSelecionado == $f['codCliFor'] ? 'selected' : '' ?>>
