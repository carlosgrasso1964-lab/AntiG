...
// Carrega clientes (codCliFor > 0999)
$stmtCli = $db->query("SELECT codCliFor, apelidoCliFor, fkCliForGp FROM tbclifor WHERE codCliFor > '0999' ORDER BY apelidoCliFor");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($clienteSelecionado && $dataIni && $dataFim) {
        $dataIniSQL = dataParaSQL($dataIni);
        $dataFimSQL = dataParaSQL($dataFim);
        
        try {
            $stmtC = $db->prepare("SELECT apelidoCliFor FROM tbclifor WHERE codCliFor = ?");
            $stmtC->execute([$clienteSelecionado]);
            $clienteInfo = $stmtC->fetch();
            $clienteNome = $clienteInfo ? $clienteInfo['apelidoCliFor'] : '';
            
            // Saldo anterior (espelha PegaSaldoAnterior do VConsClientes.java — sem filtro 9.*)
            $stmtSaldo = $db->prepare("SELECT COALESCE(SUM(Valor), 0) FROM tbmovimento WHERE dtVcto < ? AND clifor = ? AND recurso = '0022'");
            $stmtSaldo->execute([$dataIniSQL, $clienteSelecionado]);
            $saldoAnterior = (float)$stmtSaldo->fetchColumn();

            // Ficha financeira completa: inclui tanto entradas (+) quanto baixas (classif 9.*, -)
            $sql = "SELECT * FROM tbmovimento WHERE dtVcto BETWEEN ? AND ? AND clifor = ? AND recurso = '0022' ORDER BY dtVcto, idMov";
            $stmtMov = $db->prepare($sql);
            $stmtMov->execute([$dataIniSQL, $dataFimSQL, $clienteSelecionado]);
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4 class="page-title mb-0"><i class="bi bi-people"></i> Clientes</h4>
</div>
    ...
    <div class="col-md-3">
        ...
        <select name="cliente" class="form-select" required>
            ...
            <?php foreach ($clientes as $c): ?>
                <option value="<?= htmlspecialchars($c['codCliFor']) ?>" <?= $clienteSelecionado == $c['codCliFor'] ? 'selected' : '' ?>>
                    <?= htmlspecialchars($c['apelidoCliFor']) ?> (<?= htmlspecialchars($c['codCliFor']) ?>)
                </option>
            <?php endforeach; ?>
        </select>
    </div>
