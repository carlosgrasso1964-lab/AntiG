...
public class Tela_Consulta_Balance extends javax.swing.JFrame {

    Conexao conexao = new Conexao();

    public Tela_Consulta_Balance() throws SQLException {
    ...
    private void initComponents() {
        ...
        jRadioButtonEmissao.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jRadioButtonEmissaoActionPerformed(evt);
            }
        });
    ...
    private void jButtonPesquisarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonPesquisarActionPerformed
        ...
        if (jFormattedTextFieldDataIni.getText().trim().length() == 4) {
            ...
            try {
                ...
                SimpleDateFormat formato_dataEntrada = new SimpleDateFormat("dd/MM/yyyy");

                SimpleDateFormat formato_dataSaida = new SimpleDateFormat("yyyy-MM-dd");
                ...
                String dataFimString = jFormattedTextFieldDataFim.getText().trim();

                Date dataFim = formato_dataEntrada.parse(dataFimString);

                String dataFimFormatada = formato_dataSaida.format(dataFim);

                stmt.setString(1, dataFimFormatada);
                ...
                jTablePesquisa.getColumn(jTablePesquisa.getColumnName(6)).setPreferredWidth(100); // Acumulado
                ...
                jTablePesquisa.getColumnModel().getColumn(5).setCellRenderer(tab); // Mostrando no Formato Moeda
                ...
                while (rs.next()) {
                    ...
                    // Imprime o cabeçalho do relatório no topo
                    if (titulo.equals("s")) {
                    ...
                    // 3. Mudança de PRINCIPAL (Ativo / Passivo)
                    if (!prin.equals(currentPrincipal) && !prin.equals("a")) {
                    ...
                    // --- IMPRESSÃO DOS TÍTULOS EM LINHAS SEPARADAS ---
                    // A) Imprime a Coluna 0 (ATIVO / PASSIVO) em uma linha isolada
                    if (!prin.equals(currentPrincipal)) {
                        modelo.addRow(new Object[]{currentPrincipal, "", "", "", "", "", ""});
                        prin = currentPrincipal;
                    }

                    // B) Imprime a Coluna 1 (ATIVO CIRCULANTE, etc.) na linha seguinte
                    if (!sub.equals(currentSub)) {
                        modelo.addRow(new Object[]{"", currentSub, "", "", "", "", ""});
                        sub = currentSub;
                    }

                    // C) Imprime a Coluna 2 (Cabeça do Grupo / DISPONÍVEL) na linha seguinte, se houver troca de prefixo
                    ...
                    // D) Imprime a Coluna 3 (Nome da Conta: Bancos, Caixa, etc.) na linha seguinte
                    if (!cta.equals(currentConta)) {
                        modelo.addRow(new Object[]{"", "", "", currentConta, "", "", ""});
                        cta = currentConta;
                    }

                    // E) Imprime apenas os DADOS DO ITEM (Nome do Banco, Valores)
                    modelo.addRow(new Object[]{
                        "",
                        "",
                        "",
                ...
                // Consulta especial para o saldo correto de Contas à Pagar em períodos antigos
                ...
                // 1. Prepara e executa a consulta especial (sqlCP)
                String dataIniString = jFormattedTextFieldDataIni.getText().trim();

                Date dataIni = formato_dataEntrada.parse(dataIniString);

                String dataIniFormatada = formato_dataSaida.format(dataIni);
                ...
                PreparedStatement stmtCP = con.prepareStatement(sqlCP);

                stmtCP.setString(1, dataFimFormatada);

                stmtCP.setString(2, dataIniFormatada);
                ...
                if (jRadioButtonVcto.isSelected()) {
                ...
                } else if (jRadioButtonEmissao.isSelected()) {

                    sql2 = "SELECT SUM(CASE WHEN nome_P = 'RECEITAS' THEN Valor ELSE 0 END) AS TotalReceitas, "
                            + "SUM(CASE WHEN nome_P = 'DESPESAS' THEN Valor ELSE 0 END) AS TotalDespesas "
                            + "FROM viewmovrd WHERE dtEmi BETWEEN ? AND ? AND nome_P <> 'NULO' "
                            + "  AND vRecurso NOT IN ('2.001.004');";
                    ...
                    sql2 = "SELECT SUM(CASE WHEN nome_P = 'RECEITAS' THEN Valor ELSE 0 END) AS TotalReceitas, "
                            + "SUM(CASE WHEN nome_P = 'DESPESAS' THEN Valor ELSE 0 END) AS TotalDespesas "
                            + "FROM viewmovrd WHERE dtApr BETWEEN ? AND ? AND nome_P <> 'NULO' "
                            + "  AND vRecurso NOT IN ('2.001.004');";
                ...
                dataIniString = jFormattedTextFieldDataIni.getText().trim();

                dataIni = formato_dataEntrada.parse(dataIniString);

                dataIniFormatada = formato_dataSaida.format(dataIni);

                stmt2.setString(1, dataIniFormatada);

                stmt2.setString(2, dataFimFormatada);
                ...
                //double atErratico = contaTotais.getOrDefault("BANCOS", 0.0) + contaTotais.getOrDefault("CAIXA", 0.0) + contaTotais.getOrDefault("CONTA_APLICAÇÕES_C._PRAZO", 0.0);

                /*double atErratico = contaTotais.getOrDefault("BANCOS", 0.0)
...
//                        + contaTotais.getOrDefault("CONTA_APLICAÇÕES_C._PRAZO", 0.0);
                ...
                //                System.out.println("passivoTotalCirculanteENaoCirculante: " + passivoTotalCirculanteENaoCirculante);
                //                System.out.println("SubPassivo: " + SubPassivo);
                //                System.out.println("atErratico: " + atErratico);
                //                System.out.println("paErratico: " + paErratico);
                //                System.out.println("ST: " + ST);
                //                System.out.println("Ativo Circulante: " + atCirculante);
    ...
    private void jRadioButtonEmissaoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jRadioButtonEmissaoActionPerformed
        // TODO add your handling code here:
    }//GEN-LAST:event_jRadioButtonEmissaoActionPerformed
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Tela_Consulta_Balance.class
        ...
        java.awt.EventQueue.invokeLater(() -> {
            ...
            try {
                ...
                new Tela_Consulta_Balance().setVisible(true);

            } catch (SQLException ex) {

                Logger.getLogger(Tela_Consulta_Balance.class
