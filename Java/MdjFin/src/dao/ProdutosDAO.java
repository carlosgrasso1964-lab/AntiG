...
public class ProdutosDAO {
    ...
    public void salvar(Produtos p) throws SQLException {
        String sql = """
            ...
            (descricao, preco, qtd_estoque, for_id, estoqueMinimo, estoqueMaximo, 
             precoMedioCusto, preco_venda, categoria_id) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """;
    ...
    public void atualizar(Produtos p) throws SQLException {
        String sql = """
            ...
            descricao = ?, preco = ?, qtd_estoque = ?, estoqueMinimo = ?, 
            estoqueMaximo = ?, precoMedioCusto = ?, preco_venda = ?, 
            for_id = ?, categoria_id = ? 
            WHERE id = ?
            """;
    ...
    public Produtos buscarPorIdComCusto(int id) throws SQLException {
        ...
        String sql = "SELECT p.*, p.precoMedioCusto AS precoMedioCusto FROM tb_produtos p WHERE p.id = ?";
    ...
    public void entradaEstoque(Connection con, int produtoId, double qtdEntrada, double custoTotal) throws SQLException {
        String sql = """
            UPDATE tb_produtos 
            SET qtd_estoque = qtd_estoque + ?,
                precoMedioCusto = ((precoMedioCusto * qtd_estoque) + ?) / (qtd_estoque + ?)
            WHERE id = ?
            """;
    ...
    public double calcularPrecoMedioCusto(int produtoId, double qtdEntrada, double subtotal) throws SQLException {
        String sql = "SELECT qtd_estoque, precoMedioCusto FROM tb_produtos WHERE id = ?";
        ...
        try (Connection con = Conexao.faz_conexao(); PreparedStatement ps = con.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    double precoMedioAtual = rs.getDouble("precoMedioCusto");
