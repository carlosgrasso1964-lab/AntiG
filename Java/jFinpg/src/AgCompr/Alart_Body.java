...
public class Alart_Body extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel5.setIcon(new javax.swing.ImageIcon(getClass().getResource("/AgCompr/74554659-audio-symbol-with-speaker-volume-icon-inside-round-silver-and-black-button-emblem-on-black-backgroun.jpg"))); // NOI18N
    ...
    public void showTableData() {
        try {
            Connection con;
            con = Conexao.faz_conexao();
            String sql = "SELECT * FROM testbirth ORDER BY EXTRACT(MONTH FROM TO_DATE(\"Bdate\", 'YYYY-MM-DD')), EXTRACT(DAY FROM TO_DATE(\"Bdate\", 'YYYY-MM-DD'));";
            pst = (PreparedStatement) con.prepareStatement(sql);
            rs = pst.executeQuery();
    ...
    public void alart() {
        ...
        final boolean[] noOneDisplayed = {false};
        ...
        Thread t = new Thread(() -> {
            while (true) {
                ...
                for (int i = 0; i < jTable1.getRowCount(); i++) {
                    if (currentDateString.equals(jTable1.getValueAt(i, 1).toString().substring(5)) && flag == 1) {
                        mp3.play();
                        noOneDisplayed[0] = false; // Reset the flag when a match is found
                    } else {
                        ct++;
                    }
                }

                if (ct == jTable1.getRowCount() && !noOneDisplayed[0]) {
                    JOptionPane.showMessageDialog(null, "No one");
                    noOneDisplayed[0] = true; // Set the flag to avoid repeated messages
                }
