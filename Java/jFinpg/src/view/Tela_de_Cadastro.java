...
public class Tela_de_Cadastro extends javax.swing.JFrame {
    ...
    private void btAbrirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btAbrirActionPerformed
        if (tfBusca.getText().isEmpty()) {
            ...
            try {
                // Tentar converter o texto de tfBusca para um número inteiro
                int id = Integer.parseInt(tfBusca.getText());
                ...
                JOptionPane.showMessageDialog(null, "Informe um Id válido (número inteiro).");
            } catch (SQLException ex) {
                Logger.getLogger(Tela_de_Cadastro.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    ...
    private void btnAtualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAtualizarActionPerformed
        if (tfId.getText().isEmpty() || tfUsuario.getText().isEmpty() || pfSenha.getPassword().length == 0) {
            JOptionPane.showMessageDialog(null, "Informe o Id, Usuário e Senha para atualização.");
        } else {
            try {
                String senha = String.valueOf(pfSenha.getPassword());
                senha = Criptografar.encriptografar(senha);

                // Convertendo tfId para Integer (assumindo que id é do tipo integer no banco)
                int id = Integer.parseInt(tfId.getText());

                try (Connection con = Conexao.faz_conexao(); PreparedStatement stmt = con.prepareStatement("UPDATE dados_senhas SET usuario=?, senha=? WHERE id=?")) {

                    stmt.setString(1, tfUsuario.getText());
                    stmt.setString(2, senha);
                    stmt.setInt(3, id); // Utilizando setInt para o parâmetro id
                ...
                JOptionPane.showMessageDialog(null, "Erro ao atualizar usuário/senha: " + ex.getMessage());
                Logger.getLogger(Tela_de_Cadastro.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
